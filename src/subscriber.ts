...
const FLAG_MAP: Record<FlagType, number> = {
...
export class Subscriber {
  ...
  async getSubscription(publisher: `0x${string}`): Promise<Subscription> {
    ...
    const raw: any = await this.client.streamSubscription.read.getSubscription([addr, publisher]);
    const remaining = await this.client.streamSubscription.read.remainingBudget([addr, publisher]) as bigint;
  ...
  async isActive(publisher: `0x${string}`): Promise<boolean> {
    const addr = this.client.account!;
    return await this.client.streamSubscription.read.isActive([addr, publisher]) as boolean;
  }
